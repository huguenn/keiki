<div class="js-cart-item {% if item.product.is_non_shippable %}js-cart-item-non-shippable{% else %}js-cart-item-shippable{% endif %} {% if cart_page %}row align-items-md-center{% else %}cart-item form-row{% endif %}" data-item-id="{{ item.id }}" data-store="cart-item-{{ item.product.id }}">
  ...
  <div class="{% if cart_page %}col-9 col-md-10 pl-0 pl-md-3{% else %}col-10 d-flex align-items-center{% endif %}">
    <div class="{% if cart_page %}row align-items-md-center{% else %}w-100{% endif %}">
      {# Cart item name #}
      <h6 class="{% if cart_page %}col-10 col-md-5 h4-md font-weight-normal mb-3 mb-md-0{% else %}cart-item-name{% endif %}">
        <a href="{{ item.url }}">
          {{ item.short_name }}
        </a>
      ...
      <div class="cart-item-quantity {% if cart_page %}col-7 col-md-3 text-center{% endif %}">
        {% embed "snipplets/forms/form-input.tpl" with{
          ...
          form_control_container_custom_class: 'col'} %}
            ...
            <div class="row m-0 align-items-center ">
              <span class="js-cart-quantity-btn form-quantity-icon btn" onclick="LS.minusQuantity({{ item.id }}{% if not cart_page %}, true{% endif %})">
            ...
            {% block input_append_content %}
              ...
              <span class="js-cart-quantity-btn form-quantity-icon btn" onclick="LS.plusQuantity({{ item.id }}{% if not cart_page %}, true{% endif %})">
      ...
      {# Cart item subtotal #}
      <h5 class="js-cart-item-subtotal {% if cart_page %}col-5 col-md-2 text-right text-md-center mb-0 mt-2 h4-md{% else %}cart-item-subtotal h6{% endif %}" data-line-item-id="{{ item.id }}">{{ item.subtotal | money }}</h5>
    </div>
  </div>

  {# Cart item delete #}
  <div class="cart-item-delete {% if cart_page %}position-relative-md col-auto col-md-1{% else %}col-1{% endif %} text-right">
    <button type="button" class="btn {% if cart_page %}h6 h5-md mb-0{% endif %}" onclick="LS.removeItem({{ item.id }}{% if not cart_page %}, true{% endif %})">
